...
| δλ formalism: separating frame-lag from decoherence progress | 207, 1056 | **Paper 2A §2.2** (`paper2_section_2.2_delta_lambda_DRAFT.md`) | ✅ DRAFTED — G_AB(λ) = G^M + G^Σ + λG^cross decomposition, frame-lag EOM, inverse-metric to O(λ²). Sub-item: λ first-principles derivation (ledger #1). |
| Coordinate-invariant Markov transition criterion | 207, 1056 | **Paper 2A §2.3** (`paper2_section_2.3_markov_transition_DRAFT.md`) | ✅ DRAFTED — R_Markov = λ‖G^cross‖/(‖G^M‖+‖G^Σ‖), threshold ε, timescale τ_adapt vs τ_D; norm convention lock deferred to Paper 2B §3. |
| Mixed-states Born rule via purification; T_AB compatibility under dimension-changing embeddings | 513, 1056 | **Paper 2A §2.4** (`paper2_section_2.4_mixed_state_born.md`) | ✅ DRAFTED (PATCHED version exists) — Tr(ρΠ_i) via purification; cyclicity shows purification-independence (covers line 513 T_AB compatibility under Σ → Σ'). |
