...
|---|---|---|---|---|
| $T_{AB}$ | Information tensor on $\Sigma$: $T_{AB} = G_{AB} + \Omega_{AB}$ | $Q_{AB}$ | Promoted information tensor on $M \times \Sigma$: $Q_{AB} = G_{AB} + i F_{AB}$ | P2 extends to full $M \times \Sigma$; $\Omega_{AB}$ replaced by imaginary part $i F_{AB}$ |
| $G_{AB}$ | Fubini-Study metric on $\Sigma$ | $G_{ab}$ block of $G_{AB}$ | FS metric restricted to $\Sigma \subset M \times \Sigma$ | Identical formula; domain enlarged |
| $\Omega_{AB}$ | Frame-bundle curvature on $\Sigma$ (antisymmetric, real) | $F_{ab}$ block of $F_{AB}$ | Berry curvature restricted to $\Sigma \times \Sigma$ block | Same geometric content; P2 uses imaginary notation |
| *(none)* | — | $T_{M\Sigma} = G_{\mu a}$ | Symmetric coupling tensor, $(M,\Sigma)$ off-diagonal block of $\mathrm{Re}[Q_{AB}]$ | New object; no P1 analog. Drives frame-drag $\alpha = 3/2$ [Paper 2B] |
| *(none)* | — | $F_{\mu a}$ | Antisymmetric cross-sector Berry curvature, $(M,\Sigma)$ block of $\mathrm{Im}[Q_{AB}]$ | New object; encodes entanglement-mediated coupling |
| $\lambda$ | Decoherence trajectory coordinate along a curve in $\Sigma$ | $\lambda(x, \xi)$ | Scalar field on $M \times \Sigma$; same physical quantity promoted to full field | Restriction: $\lambda|_{x_0, \gamma(t)}$ recovers P1 usage exactly |
| $\Phi: \Sigma \to \mathcal{PH}$ | Frame bundle map from $\Sigma$ into projective Hilbert space | $\Phi: M \times \Sigma \to \mathcal{PH}$ | Same map; domain enlarged to $M \times \Sigma$ | Pointwise in $x \in M$, P2 $\Phi$ recovers P1 $\Phi$ |
| $\delta\lambda$ | Infinitesimal decoherence displacement | $\delta\lambda(x, \xi)$ | Same quantity as a 1-form on $M \times \Sigma$ | Identical formula; P2 adds integrability conditions |
| $\rho(t)$ | Density matrix evolving in time | $\rho(x, t)$ | Density matrix as a field over $M$ | Adds spatial dependence |
| $\Gamma_{\mathrm{dec}}$ | Decoherence rate (scalar) | $\Gamma_{\mathrm{dec}}(\eta)$ | Decoherence rate as function of radial coordinate [Paper 2B] | Generalized; recovers scalar in flat-$\Sigma$ limit |

**Key clarification**: $T_{AB}$ (Paper 1) is the $(a,b)$ block of $Q_{AB}$ restricted to $\Sigma$. $T_{M\Sigma} \equiv G_{\mu a}$ is the $(M,\Sigma)$ coupling block of $Q_{AB}$ — a new object with no Paper 1 analog, requiring both $M$ and $\Sigma$ to be simultaneously present.
...
<!-- SUPERSEDED BY §4 v2 BELOW — REMOVE IN COMPRESSION PASS -->
...
# §4 Equations of Motion on M × Σ — v2 (2026-04-08)
