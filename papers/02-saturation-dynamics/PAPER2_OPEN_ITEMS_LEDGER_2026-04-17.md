...
| 23 | ~~**Factor-of-6 in Path C Λ_eff**~~ → **α_geom value (~70% derived)** | RC-2.5, RC-6, RC-7 | **PARTIAL (RC-7, 2026-04-18).** The "factor of 6" was an artifact of a π error in RC-2 (RC-6). Correct formula is Ω_drag = α_geom × c_Γ² (Eq. RC6.1). RC-7 attempted first-principles derivation: identified four candidate expressions; the physically natural one is α_geom = N₀²·I₆/I₂ = 10√2/(3π) ≈ 1.5005 (zero-mode energy density with graviton measure, normalized to graviton zero-mode norm). This is within 0.035% of 3/2 but **not** exactly 3/2. Gives c_Γ ≈ 0.678 (matches Paper 2A claim). Definitive selection requires full second-order expansion of 5D Einstein–Hilbert action in δg_μr (RC-8 scope). Observational constraint α_geom × c_Γ² = 0.69 is robust to the 1.5005-vs-1.5000 ambiguity. See `RC7_ALPHA_GEOM_DERIVATION_ATTEMPT_2026-04-18.md`. |
...
- RC-4: #22 ($k_c^{\rm eff}$) gates 2C §RC1.4 full predictions; #23 upgraded to PARTIAL (RC-7) — α_geom candidate expression identified, full derivation pending; c_Γ still open.
...
- **RC-7 findings (2026-04-18):** α_geom derivation pushed to ~70%. T_μr is a metric perturbation (not a Maxwell field) — standard KK vector formula does not apply. Zero-mode profile ψ₀ = N₀A² from RC-3 plus graviton measure A²dr gives four candidate expressions for α_geom. Physically natural candidate: **α_geom = N₀²·I₆/I₂ = 10√2/(3π) ≈ 1.5005** (zero-mode energy density with graviton measure, normalized to graviton zero-mode norm). Within 0.035% of exactly 3/2 — suggestive of deep relation but not proven. Gives c_Γ ≈ 0.678. Full 5D EH expansion to second order in δg_μr (RC-8) required to definitively select among candidates. Paper 2A §3.3.5.3 "α = 3/2 exact from CP¹" should be rewritten using RC-7.6 language. See `RC7_ALPHA_GEOM_DERIVATION_ATTEMPT_2026-04-18.md`.
