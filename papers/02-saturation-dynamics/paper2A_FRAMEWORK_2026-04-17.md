...
>
> **Requirement 1 — Precise symmetry group.** Define the coherence-frame group $\mathcal{G}_{\text{CR}}$ acting on $\mathcal{D}(\mathcal{H})$: the allowed frame rotations, reference changes, and picture transformations of the HCR framework. Show that $\mathcal{G}_{\text{CR}}$ acts unitarily (or at minimum unitarily up to CP maps) on the relevant state space, and that its orbit structure is rich enough — i.e., the orbits under $\mathcal{G}_{\text{CR}}$ are the same as, or contain, the orbits of the unitary group $U(n)$ restricted to the pointer-compatible sector.
>
> **Requirement 2 — Noncontextuality and regularity in HCR language.** Translate the standard hypotheses of Gleason-type theorems into coherence-frame primitives:
> - *Noncontextuality*: the probability assigned to a coherence-frame projector $|\psi\rangle\langle\psi|$ does not depend on which other commuting frame-projectors are co-measured. State this as a condition on how probabilities behave under environment splits and pointer-sector refinements.
> - *Regularity*: the probability functional $p: \mathcal{D}(\mathcal{H}) \to [0,1]$ is continuous, normalized ($p(\mathbf{I}/n) = 1/n$), and $\sigma$-additive over orthogonal alternatives in any coarse-graining consistent with the Markov criterion (§2.6).
>
> **Requirement 3 — Uniqueness lemma.** Prove (or cite an existing theorem that directly applies):
>
> *Any probability functional $p$ on the unit sphere of $\mathcal{H}$ that (i) is invariant under all allowed coherence-frame rotations, (ii) is additive over mutually orthogonal alternatives, and (iii) is normalized and continuous, must be of the form $p(\psi) = \langle\psi|M|\psi\rangle$ for some linear operator $M$; hence $p(\psi) = |\langle\phi|\psi\rangle|^2$ for a transition weight.*
>
> At present the dephasing model provides Requirement 3 for $\mathcal{H} = \mathbb{C}^2$ under the specific $\mathcal{G}_{\text{CR}}$ generated by $\sigma_z$-pointer symmetry. Extending to general $\mathcal{H}$ and the full $\mathcal{G}_{\text{CR}}$ is the open step. The most direct path would be to map the HCR coherence-frame rotations onto the envariance construction of Zurek (2003), which requires only that the system-environment entangled state be invariant under swap symmetries — a condition likely satisfied by the pointer-sector structure established in §2.5.6.
